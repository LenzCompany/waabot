...
module.exports = async (sock, m) => {
    try {
        ...
        //DATABASE MESSAGE
        //MENYIMPAN SEMUA PESAN YANG MASUK KEPADA BOT!
        const dbdb = JSON.parse(fs.readFileSync('./message_database.json'))
        ...
        if (m.message) {
            ...
            await fs.writeFileSync('./message_database.json', JSON.stringify(dbdb))
        }
        ...
        switch (command) {
            ...
            case "pesan":
            case "pesan-masuk": {
                if (!isCreator) return reply(`*Hanya Bisa Digunakan Oleh Pemilik Bot*`)
                let pesan = require('./message_database.json')
                let alok = pesan.map(e => `Nama : ${e.nama}\nNomor : ${e.nomor}\nPesan : ${e.pesan}\nWaktu Mengirim : ${e.time}`).join('\n\n\n')
                reply(alok)
            }
            break
            case "menu": case "help": {
                ...
                NAMA : ${pushname}
                ID : ${db.get(`user.${m.sender}`).uid}


*OWNER*
${prefix}pesan - Pesan Masuk
